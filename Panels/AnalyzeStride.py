...
class AnalyzeStridePanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.px_speed = configs['px_speed']
    ...
    def onAnalysisTypeRadioBox(self, e):
        ...
        if self.analysis_type == "Treadmill":
            ...
            if self.pixels_per_cm == '':
                wx.MessageBox('For spontaneous walking analysis, please supply a value for "pixels_per_cm" in the config.yaml file!',
                              'Error',
        ...
        try:
            if self.df is not None:
                ...
                if self.pixels_per_cm != '':
                    ...
                    wx.MessageBox('For spontaneous walking, please be sure to supply a pxl-cm conversion ratio in the config file!',
                                  'Note',
                                  wx.OK | wx.ICON_INFORMATION)
        ...
        if self.has_imported_file:
            ...
            if self.analysis_type == "Treadmill":
                ...
                if self.method_selection == 'Semi-automated':
                    ...
                    self.est_cm_speed, self.est_px_speed, self.est_pixels_per_cm, self.est_px_to_cm_speed_ratio = KinematicsFunctions.estimate_speed(
                        self.df, 'toe', self.cm_speed, self.px_to_cm_speed_ratio, self.frame_rate, self.px_speed, self.pixels_per_cm, self.right_to_left)
    ...
    def BulkExtractParameters(self, e):
        ...
        for i, file in enumerate(self.files):
            ...
            if self.analysis_type == "Treadmill":
                ...
                parameters_truncated = KinematicsFunctions.return_continuous(
                    parameters, 5, True, pd_dataframe_coords, bodyparts, is_stance, plot_path)

            KinematicsFunctions.make_parameters_output(os.path.join(self.output_path, f'parameters_{file}'), parameters)
            KinematicsFunctions.make_parameters_output(os.path.join(
                self.output_path, f'continuous_strides_parameters_{file}'), parameters_truncated)
    ...
    def ExtractParameters(self, e):
        ...
        KinematicsFunctions.make_parameters_output(os.path.join(os.path.dirname(
            self.output_path), f'continuous_strides_parameters_{self.filename}.csv'), parameters_truncated)
