...
def estimate_speed(pd_dataframe, bodypart, cm_speed, px_to_cm_speed_ratio, frame_rate, px_speed, pixels_per_cm, right_to_left=True):
    ...
    px_to_cm_speed_ratio = px_to_cm_speed_ratio
    ...
    if end_stride == start_stride:
        ...
        try:
            if plot:
                ...
                continuous_stickplot(pd_dataframe_coords, bodyparts, is_stance, x_coords,
                                     y_coords, start_frame, end_frame, filename)
        except IndexError:
            ...
            if starts[i] not in starts_included:
                ...
                if included_index <= len(starts_included)-5:
                    ...
                    if included_index <= len(starts_included)-10:
                        toe_xs = parse_stride_lists(smooth_toe_x, starts_included,
                                                    ends_included, included_index, 10, True)
                        ...
                        toe_ys = parse_stride_lists(smooth_toe_y, starts_included,
                                                    ends_included, included_index, 10, True)
    ...
    for direction in ['L', 'R', '']:
        if f'toe{direction} x' in pd_dataframe.columns:
            ...
            if dragging_filter and \
                ...
                # use this plus SD (i.e., lower in space) as treadmill y coord, to calculate step height
                treadmill_y = float(stance_threshold +
            ...
            for i in range(len(starts)):
                ...
                if starts[i] not in starts_included:
                    ...
                    if included_index <= len(starts_included)-5:
                        ...
                        toe_xs = parse_stride_lists(smooth_toe_x, starts_included,
                                                    ends_included, included_index, 5, True)
                        pairwise_dtw_res_x_5 = pairwise_dtw(toe_xs)
                        dtw_x_plane_5_mean = np.mean(pairwise_dtw_res_x_5)
                        dtw_x_plane_5_sd = np.std(pairwise_dtw_res_x_5)
...
def collect_filtered_coords(filt_corrected_df, bodyparts):
    ...
    x_coords = filt_corrected_df[[f'{bodyparts[0]} x', f'{bodyparts[1]} x',
                                  f'{bodyparts[2]} x', f'{bodyparts[3]} x', f'{bodyparts[4]} x', f'{bodyparts[5]} x']]
    y_coords = filt_corrected_df[[f'{bodyparts[0]} y', f'{bodyparts[1]} y',
                                  f'{bodyparts[2]} y', f'{bodyparts[3]} y', f'{bodyparts[4]} y', f'{bodyparts[5]} y']]
